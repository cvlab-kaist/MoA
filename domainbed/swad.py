...
class LossValley(SWADBase):
    ...
    def update_and_evaluate(self, segment_swa, val_acc, val_loss, prt_fn):
        ...
        if not self.is_converged:
            ...
            # print("untilmin_segment_swa:")
            # print([p for p in untilmin_segment_swa.parameters()][0].device)
            if min_idx == 0:

                self.converge_step = self.converge_Q[0].end_step
                self.final_model = swa_utils.AveragedModel(untilmin_segment_swa, rm_optimizer=True)
                # print([p for p in self.final_model.parameters()][0].device)
